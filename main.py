...
parser.add_argument('-t', '--threshold', dest='threshold', action='store_true', required=False, help='specify thresholding value')
...
def main():
    ...
    # Data loading code
    train_dataset, val_dataset, test_dataset, attr_num, description = Get_Dataset("jagalchi", args.approach)
...
def test(val_loader, model, attr_num, description):
    ...
    if args.threshold:
        max_thress = np.load(f"./model/jagalchi/max_thres_{args.start_epoch}")
    else:
        max_thress = np.zeros(attr_num)
        max_thress += 50
    ...
    for i, _ in enumerate(val_loader):
        ...
        with torch.no_grad():
            ...
            tol = tol + batch_size
            output = torch.sigmoid(output.data).cpu().numpy()       # (batch_size, attr)

            # output = np.where(output > 0.5, 1, 0)
            for it in range(attr_num):
                output[:, it] = np.where(output[:, it] > max_thress[it] / 100, 1, 0)
    ...
    for it in range(attr_num):
        if pos_tol[it] == 0:
            ...
            pos_mA = 1.0*pos_cnt[it]/pos_tol[it]
        if neg_tol[it] == 0:
            neg_mA = 1.0
        else:
            neg_mA = 1.0*neg_cnt[it]/neg_tol[it]
...
def save_checkpoint(state, epoch, prefix, filename='.pth.tar'):
    ...
    directory = "./model/" + args.experiment + '/'
    ...
    print(directory + prefix + '_' + str(epoch) + filename)
